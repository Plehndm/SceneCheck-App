...
const SC_ME = {
  ...
  events_attended: 12,
  city: 'Irvine, CA',
  picture: null, // data URL, set by user
  privacy: 'public', // 'public' | 'private'
};

// Accounts the user manages (Instagram-style account switcher).
// "me" is their personal account. The rest are organization accounts.
// An org account is treated like a single Instagram/TikTok-style page:
// one account = the entire organization. No per-user roles are tracked.
const SC_MY_ACCOUNTS = [
...
// Other org accounts you don't manage but can follow
const SC_ORGS = [
...
// Incoming friend requests waiting for your approval (only relevant when your privacy='private')
const SC_FRIEND_REQUESTS = [
  { id:'fr1', personId:'p4', when:'2h ago', note:"saw you at the Tuesday run — let's link up" },
  { id:'fr2', personId:'p6', when:'yesterday', note:null },
];

const SC_INTERESTS_SUGGESTED = [
...
const SC_INTERESTS_DETAILS = Object.fromEntries(SC_INTERESTS_SUGGESTED.map(i => [i.tag, i]));
// also for "your current"
SC_INTERESTS_DETAILS['uci']         = { tag:'uci', others: 18403, desc:"University of California, Irvine. The campus tag covers anything happening at or organized through UCI — clubs, classes, study groups, intramural sports.", similar:['anteaters','uci-clubs','irvine'] };
...
// kind: 'yours' = hosted by you (or one of your managed accounts)
//       'friend' = hosted by a friend
//       'org'    = hosted by an org you follow
//       'recommended' = app-discovered
// hostId points to person/org account hosting (when applicable)
const SC_EVENTS = [
  // your event (hosted by your personal account)
  ...
  // overlaps your Sat AM ride — used to demo pre-flight conflict warnings
  { id:'e9', kind:'org',    hostId:'orgA', title:'Saturday Sunrise Yoga',        host:'Mind & Movement', interests:['yoga','uci'], when:'Sat May 9 · 8:00 AM', endTime:'9:00 AM', where:'Aldrich Park lawn', attendees: 11, cap: 30, rating: 4.8, x: 0.48, y: 0.46, desc:"Posted by @mindmovement. 60-minute all-levels flow on the lawn. Bring a mat — we have a few spares." },
  // friend events
  ...
  // org-hosted (you follow these orgs → you got notified)
  { id:'e7', kind:'org',    hostId:'orgA', title:'Beginner Bouldering Night',    host:'TopOut Climbing', interests:['climbing'], when:'Fri May 8 · 7:30 PM', endTime:'10:00 PM', where:'TopOut Irvine', attendees: 28, cap: 40, rating: 4.7, x: 0.20, y: 0.66, desc:"Posted by @topoutirvine — followers got notified. Free top-rope intro. Day pass + rentals $18." },
  { id:'e8', kind:'org',    hostId:'orgD', title:'Tuesday Group Run · 5K',       host:'Anteater Run Club', interests:['running','uci'], when:'Tue May 12 · 6:00 PM', endTime:'7:30 PM', where:'Aldrich Park flagpole', attendees: 17, cap: 50, rating: 4.6, x: 0.50, y: 0.42, desc:"Posted by @anteaterrun. Couch-to-5K pace group + open pace group. Stretch + boba after." },
  // recommended (app-created)
  { id:'e4', kind:'recommended', hostId:null, title:'Pickup Soccer @ Aldrich', host:'App-created', interests:['running','uci'], when:'Wed May 6 · 5:00 PM', endTime:'7:00 PM', where:'Aldrich Park East Field', attendees: 22, cap: 30, rating: null, x: 0.46, y: 0.55, desc:"Open pickup, 5v5 rotating. Cleats not required. Pulled from the UCI Rec calendar." },
...
const SC_EVENT_BY_ID = Object.fromEntries(SC_EVENTS.map(e => [e.id, e]));

// ── DRAFTS (seed) ─────────────────────────────────────────────────
// Saved-but-unpublished events authored by the user. New drafts get pushed
// here whenever the publish-failure recovery flow saves the in-progress form.
// Each draft is a partial form snapshot with a generated id + savedAt label
// and the step the user had reached, so resuming returns them to that point.
const SC_DRAFTS_SEED = [
  {
    id: 'd_seed1',
    ...
    form: {
      ...
      timeStart: '7:30 AM',
      timeEnd: '11:00 AM',
      location: 'Anteater Plaza → Newport Coast',
      cap: 10,
      interests: ['biking','uci'],
      visibility: 'public',
      minSubs: 2,
      addToCalendar: true,
      autoGroupChat: true,
...
// these by id alongside ids from SC_EVENTS, so a host's full rating history
// surfaces — not just events that happen to still be on the map.
const SC_PAST_EVENTS = [
...
// Combined lookup — current events + past events — for the ratings screen.
const SC_ANY_EVENT_BY_ID = Object.fromEntries(
  [...SC_EVENTS, ...SC_PAST_EVENTS].map(e => [e.id, e])
);

// Dummy reviews. Each review is FOR an event, and the rating attaches to the
// event's host. Filter by `hostId` to get a host's review history; group by
// `eventId` to slice by event. `reviewerId` resolves to SC_PEOPLE/SC_ME for
// the avatar + name; reviews from accounts you can't see (or external folks
// who never made a profile) fall back to `reviewerName`.
const SC_REVIEWS = [
  // ── Alex (me) — host of e1 + 3 past rides ──
  { id:'r01', eventId:'e1',  hostId:'me', reviewerId:'p3', rating:5, when:'last Saturday',
    text:"chill pace, great regroup at the bridge, alex actually checked in on everyone. easiest 14 miles I've done." },
  { id:'r02', eventId:'e1',  hostId:'me', reviewerId:'p1', rating:5, when:'last Saturday',
    text:"coffee stop made it. would join again." },
  { id:'r03', eventId:'pe1', hostId:'me', reviewerId:'p4', rating:4, when:'Apr 27',
    text:"newport coast is brutal but alex paced it well. start was a touch chaotic — maybe stagger the group next time." },
  { id:'r04', eventId:'pe1', hostId:'me', reviewerId:'p2', rating:5, when:'Apr 27',
    text:"sweep rider all the way. nobody got dropped. 10/10." },
  { id:'r05', eventId:'pe2', hostId:'me', reviewerId:'p1', rating:4, when:'Apr 20',
    text:"recovery pace was real. thanks for not turning it into a hammerfest." },
  { id:'r06', eventId:'pe3', hostId:'me', reviewerId:'p5', rating:3, when:'Apr 13',
    text:"6am is 6am. ride was fine. would prefer a 7am start." },

  // ── Sasha (p3) — cooking club ──
  { id:'r07', eventId:'e2',  hostId:'p3', reviewerId:'p1', rating:5, when:'Apr 24',
    text:"sasha is a teaching natural. the pleating tutorial alone was worth it." },
  { id:'r08', eventId:'e2',  hostId:'p3', reviewerId:'p5', rating:5, when:'Apr 24',
    text:"left full + with leftovers. somehow under budget." },
  { id:'r09', eventId:'pe4', hostId:'p3', reviewerId:'me', rating:5, when:'Apr 26',
    text:"hand-pulled noodles is HARD. sasha made it look easy and patient. amazing." },
  { id:'r10', eventId:'pe4', hostId:'p3', reviewerId:'p2', rating:4, when:'Apr 26',
    text:"great event. kitchen got a little cramped — maybe cap at 12 next time." },
  { id:'r11', eventId:'pe5', hostId:'p3', reviewerId:'p4', rating:2, when:'Apr 12',
    text:"the food was great but it ran 90 min over and parking at mesa was a nightmare." },

  // ── Jordan (p2) — climbing ──
  { id:'r12', eventId:'e3',  hostId:'p2', reviewerId:'p3', rating:5, when:'Apr 18',
    text:"jordan walked the first-timers through every move. unintimidating. perfect intro." },
  { id:'r13', eventId:'e3',  hostId:'p2', reviewerId:'p5', rating:4, when:'Apr 18',
    text:"loved it. shoes felt huge on me — heads up that the gym only has half sizes." },
  { id:'r14', eventId:'pe6', hostId:'p2', reviewerId:'me', rating:5, when:'Apr 6',
    text:"first outdoor climb of my life. jordan set up safe top-ropes and explained every knot. unreal day." },
  { id:'r15', eventId:'pe7', hostId:'p2', reviewerId:'p4', rating:3, when:'Mar 29',
    text:"good routes but the group was way more advanced than i expected. should mark 'intermediate' more clearly." },

  // ── Maya (p1) ──
  { id:'r16', eventId:'pe8', hostId:'p1', reviewerId:'me', rating:5, when:'Apr 21',
    text:"4 cafes in 3 hours and i still want more. maya's route was perfect — bonus that she knew the baristas." },
  { id:'r17', eventId:'pe8', hostId:'p1', reviewerId:'p3', rating:4, when:'Apr 21',
    text:"loved this. 4th stop felt like one too many — but no notes otherwise." },
  { id:'r18', eventId:'pe9', hostId:'p1', reviewerId:'p5', rating:5, when:'Apr 7',
    text:"saw maya's senior thesis in person. friendly, thoughtful host, fed us snacks." },

  // ── Theo (p4) ──
  { id:'r19', eventId:'pe10',hostId:'p4', reviewerId:'me',   rating:4, when:'Apr 23',
    text:"theo's track workouts are no-nonsense. clear pacing, good warmup." },
  { id:'r20', eventId:'pe10',hostId:'p4', reviewerId:'p1',   rating:3, when:'Apr 23',
    text:"showed up at 5:58, theo was already on lap 2. would appreciate a 5 min warmup window." },
  { id:'r21', eventId:'pe11',hostId:'p4', reviewerId:'p2',   rating:5, when:'Apr 16',
    text:"theo paced the whole 12mi with me. genuine human." },

  // ── Priya (p5) ──
  { id:'r22', eventId:'pe12',hostId:'p5', reviewerId:'p1',   rating:5, when:'Apr 19',
    text:"priya curated a beautiful listening set. learned a lot about carnatic structure." },
  { id:'r23', eventId:'pe13',hostId:'p5', reviewerId:'me',   rating:4, when:'Apr 5',
    text:"great study group. priya kept it on track. one off-topic discussion went long." },

  // ── TopOut (orgA) ──
  { id:'r24', eventId:'e7',  hostId:'orgA', reviewerId:'p2',   rating:5, when:'last Friday',
    text:"staff are pros. routes set thoughtfully for beginners. day pass is a steal at $18." },
  { id:'r25', eventId:'e7',  hostId:'orgA', reviewerId:'p3',   rating:4, when:'last Friday',
    text:"loved the night. wait for the top-rope station was long around 8pm." },
  { id:'r26', eventId:'pe14',hostId:'orgA', reviewerId:'p1',   rating:5, when:'May 1',
    text:"comp was a vibe. live commentary, real prizes, fair grading." },

  // ── Anteater Run Club (orgD) ──
  { id:'r27', eventId:'e8',  hostId:'orgD', reviewerId:'p4',   rating:5, when:'May 6',
    text:"both pace groups got real attention. boba after was a nice touch." },
  { id:'r28', eventId:'pe15',hostId:'orgD', reviewerId:'p5',   rating:4, when:'Apr 15',
    text:"the training plan is solid. could use a slow pace group for c25k folks." },
  { id:'r29', eventId:'pe15',hostId:'orgD', reviewerId:'me',   rating:5, when:'Apr 15',
    text:"showed up nervous, left signed up for the half. coaches really know what they're doing." },
];
...
// Per-chat dummy threads. Keys are SC_CHATS ids.
const SC_THREADS = {
  c1: SC_THREAD_E1,
  c2: [
...
// Quick lookup of any account (person or org) by id
const SC_ACCOUNT_BY_ID = Object.fromEntries(
  [...SC_MY_ACCOUNTS, ...SC_ORGS, ...SC_PEOPLE].map(a => [a.id, a])
);

// People who have NOT blocked you. This is the canonical list for any
// surface that displays people from your perspective — search, suggestions,
// attendee rosters, friends-of-friends, map dots, etc. A blocker doesn't
// appear in your view of the app at all.
const SC_VISIBLE_PEOPLE = SC_PEOPLE.filter(p => !p.blockedYou);
// Lookup that returns undefined when the person has blocked you, so callers
// can render an "unavailable" state instead of leaking the profile.
const SC_VISIBLE_PERSON_BY_ID = Object.fromEntries(
  SC_VISIBLE_PEOPLE.map(p => [p.id, p])
);

Object.assign(window, {
  SC_ME, SC_MY_ACCOUNTS, SC_ORGS, SC_PEOPLE, SC_FRIEND_REQUESTS,
  SC_VISIBLE_PEOPLE, SC_VISIBLE_PERSON_BY_ID,
