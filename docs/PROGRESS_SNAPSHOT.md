...
| 2026-06-02 | **Map live-status chip, profile→discover deep-link, floating web onboarding, David/Maya event date shift (migration 00046) + a stale Data API finding.** Web-only UX: the map LIVE/OFFLINE chip (`web/useOnline.ts`) now reflects **real** connection health — `navigator.onLine` *plus* a Supabase Realtime heartbeat channel (`SUBSCRIBED` = live) — instead of the browser flag alone (which stays `true` on a no-internet LAN); the profile **"Add interests"** chip deep-links to `/search?tab=interests` (Discover's Interests tab); and the FR1.3 onboarding picker gets a web variant (`app/onboarding/interests.web.tsx`) presented as a **floating dialog over a blurred, inert preview of the explore page** (native unchanged). Hosted data: migration **`00046`** shifts Maya's + the davidplehn07@gmail.com account's events one week out so they re-enter `rank_events_query`'s window — applied + **verified on the primary** (Morning Ride → 2026-06-09 published; David's "IN4MATX 43 Study Session" → 2026-06-09, still **draft**). ⚠️ **[CORRECTED 2026-06-05 — see §76.4]** A "stale Data API serving 9 frozen events" was reported here, but it was a **measurement artifact**: the verification script queried the *local* Supabase stack (a commented-out URL in `.env`), not hosted. The hosted Data API was healthy all along (286 events, the shift live); there was no read-replica/infra issue. `tsc` clean; jest 423/426 (3 pre-existing stale-fixture failures; the web surfaces are outside Jest). See §76. |
...
| 2026-06-04 | **Event preview/cover images (Eventbrite → app).** Eventbrite's schema.org Event JSON-LD carries an `image` URL (`img.evbuc.com`); verified live that all events on a discovery page have one. **Slice 1 (pipeline):** the FR6 scraper reads `it.image` (`normalizeImageUrl`: string\|array\|`{url}`, http(s) only, kept verbatim so the signed CDN transform stays valid) and ships `image_url`; migration **`00047`** adds nullable `events.image_url` + DROP/CREATEs `rank_events_query` to surface it on the discovery feed (explicit column list); `ingest-scraped` validates + inserts + self-heals it; `lib/api` `EventRow`/`transformEventRow` map `row.image_url → event.image`; `types/domain` `SCEvent` gains `image`; two mock events get Picsum previews. **Slice 2 (UI):** `WebEventListCard` full-bleed banner, web event-detail 220px hero, `SCEventCard` 110px thumbnail, native event-detail hero fills with the image (accent fallback). Every surface hides the image on load error and omits it when null (user-created events unchanged). **Hot-linked** (not re-hosted). `tsc` clean; jest 423/426; migration applied + `ingest-scraped` redeployed to hosted. Existing rows backfill on the next scrape (self-heal). (An earlier "§76.4 stale-Data-API caveat" on live images was later found to be a measurement artifact — hosted shows **143 events with covers**; see §76.4.) See §78. |
...
| 2026-06-05 | **Correction: the "stale Data API" (§76.4) was a measurement artifact, not an infra issue.** Verifying the deployed build's cover images, found that the REST checks behind the §76.4 "Data API serving 9 frozen events" finding had been reading the **commented-out local-stack URL** in `scenecheck-expo/.env` (a non-anchored regex matched `#  EXPO_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321` above the active hosted line) — so they hit the **local stack** (9 seed rows, no migrations), not hosted. A correct query against hosted (`kmlecodmifljbtzaqahm`) returns **286 events, 143 with real `img.evbuc.com` covers**, and the David/Maya date shift is live. There was never a read-replica/stale-read problem; the apparent "hosted-vs-API divergence" was just hosted (`db push`) vs local (buggy REST). Corrected §76.4 + the propagated caveats in the 2026-06-02 / 06-04 rows, §78, and TEST_PLAN §2.71. No code change. See §76.4. |
...
> **⚠️ CORRECTION (2026-06-05): the "stale Data API" finding below was a
> measurement artifact — there was NO infrastructure issue.** The verification
> script read `scenecheck-expo/.env` with a regex that matched the
> *commented-out* local-stack line (`#  EXPO_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321`)
> sitting above the active hosted line, so every REST check was hitting the
> **local Supabase stack** (9 seed events, no migrations applied), not the hosted
> project. The hosted Data API was healthy the whole time: a correct query
> returns **286 events, 143 with `image_url`** (real `img.evbuc.com` covers), and
> the David/Maya date shift is live. The `db push` probes (which DID hit hosted)
> showed 222+ rows — the "divergence" was simply hosted-vs-local, not a read
> replica. Disregard the stale-read claim, the read-replica hypothesis, and the
> "won't surface in the app" conclusion in the rest of this section; only the
> migration itself + "David's event is a draft" note still stand.
...
scraped` redeployed to the hosted project. **Live status (confirmed
2026-06-05):** the hosted project now has **286 events, 143 with real
`img.evbuc.com` covers**, and the deployed build renders them. (An earlier draft
of this note carried a "§76.4 stale-Data-API" caveat — that was a measurement
artifact, not a real issue; see §76.4.) Existing rows backfill on each
**scrape-events** GitHub Action run via the ingest self-heal.
