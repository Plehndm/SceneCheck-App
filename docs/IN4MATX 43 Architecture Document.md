...
| ---                                            | ---                                                                                                                       | ---                                                                                                                                                                                                                                      | ---                                                                                                                                                                                                                                                                                                                     |
| JavaScript<br><br>Node.js 22 LTS | \-Background scraper<br><br>\-GitHub Actions worker running<br><br>\-Global `fetch` + JSON-LD parsing (no Playwright / headless browser dependency)                       | \-Reuses the same language as the frontend, so any teammate can read and maintain the scraper<br><br>\-Node.js 22's built-in global `fetch` lets the scraper run with zero npm dependencies<br><br>\-JSON-LD is a stable structured-data contract on Eventbrite + most campus calendars, so regex extraction is sufficient | \-Source pages that are pure JS-rendered (no JSON-LD in the initial HTML) are out of reach without re-introducing a headless browser; scope today is the public listings that do publish JSON-LD                                                                                                                                       |
| ---                                            | ---                                                                                                                       | ---                                                                                                                                                                                                                                      | ---                                                                                                                                                                                                                                                                                                                     |
...
- GitHub Actions cron runs Node.js against Eventbrite, campus calendars, Meetup-style pages (parses JSON-LD; no headless browser needed)
- Each scraped event is validated then posted to the Edge Function:
- <span style="color:#188038">POST /functions/v1/ingest-scraped with apikey: {sb_secret_…} and x-ingest-token: {INGEST_TOKEN}</span>
- Body: <span style="color:#188038">{title, description, start_at, end_at, location, source_url}</span>
- The function is deployed with <span style="color:#188038">--no-verify-jwt</span> because <span style="color:#188038">sb_secret_…</span> is not a JWT; authorization is the <span style="color:#188038">x-ingest-token</span> shared secret matched server-side
...
| ---                                                        | ---                                                                                                                                                      |
| Step 2 - Worker posts each candidate                       | <span style="color:#188038">Worker</span> → POST <span style="color:#188038">/functions/v1/ingest-scraped {apikey: sb_secret_…, x-ingest-token: INGEST_TOKEN, payload: {title, description, start_at, end_at, location, source_url}}</span> (function deployed with `--no-verify-jwt`) |
| ---                                                        | ---                                                                                                                                                      |
